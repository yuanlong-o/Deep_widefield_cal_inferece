...
def nb_view_patches(Yr, A, C, b, f, d1, d2, output_file_path = None, YrA=None,
         image_neurons=None, thr=0.99, denoised_color=None, cmap='jet'):
    """
    ...
    if output_file_path: 
        bpl.output_file(filename=output_file_path, title = 'Traces')
    ...
    data = None
    if Y_r.shape[0] > 1:
        data = bokeh.layouts.layout([[slider], [bokeh.layouts.row(plot1, plot)]])
        # bpl.show(bokeh.layouts.layout([[slider], [bokeh.layouts.row(plot1, plot)]]))
    else:
        data = bokeh.layouts.row(plot1, plot)
        # bpl.show(bokeh.layouts.row(plot1, plot))
    if output_file_path: bpl.save(data)
    bpl.show(data)
