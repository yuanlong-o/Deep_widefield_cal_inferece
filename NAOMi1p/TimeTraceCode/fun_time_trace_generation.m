function fun_time_trace_generation(vol_out,nt, frate, output_dir)
...
% vol_output = importdata(sprintf('%s\\%s', input_dir, input_file));
...
spike_opts.dt = 1 / frate;   % Default sampling rate is 30 Hz
spike_opts.nt = nt + 100; % Default number of time-steps to simulate is 1000. Discard the first 100 frames.
...
outdir = sprintf('%s\\firing_rate_%g_smod_flag_%s', output_dir, spike_opts.rate, spike_opts.smod_flag);
mkdir(outdir)                        
