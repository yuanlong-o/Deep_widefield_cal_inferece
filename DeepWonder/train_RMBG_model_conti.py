...
if __name__ == '__main__':
    ...
    parser.add_argument('--GT_folder', type=str, default='GT', help="video or image")
    parser.add_argument('--input_folder', type=str, default='Input', help="")
    
    parser.add_argument('--datasets_path', type=str, default='datasets', help="dataset root path")
    parser.add_argument('--pth_path', type=str, default='RMBG_pth', help="pth file root path")
    ...
    parser.add_argument('--pth_name', type=str, default='train_20210401_1712', help='datasets size for training')
    ...
    current_time = opt.datasets_folder+'_dn'+str(opt.down_num)+'_fm'+str(opt.fmap)+'_'+datetime.datetime.now().strftime("%Y%m%d%H%M")
    ...
    pth_name = str(opt.pth_name)+'.pth'
    model_name = opt.pth_path + '//' + opt.denoise_model + '//' + pth_name
    denoise_generator.load_state_dict(torch.load(model_name))
    ...
    for epoch in range(0, opt.n_epochs):
        ...
        for index in range(len(coor_list)):
            ...
            # print('fake_B ---> ',fake_B.device)
            # print('real_B ---> ',real_B.device)
            ...
            if iteration%10 == 0:
                ...
                f.write('Dloss1 '+str(Dloss1.item())+' fake_loss '+str(fake_loss.item())+' \n')
        ...
        if (epoch+1)%1 == 0:
            ...
            noise_img1_name = pth_path + '/' + str(epoch) + '_input.tif'
            noise_img2_name = pth_path + '/' + str(epoch)  + '_gt.tif'
