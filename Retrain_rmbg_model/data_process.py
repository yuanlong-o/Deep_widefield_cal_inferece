...
def train_preprocess(args):
    ...
    im_folder2 = 'datasets//'+args.datasets_folder + '_gt'
    ...
    for im_name in list(os.walk(im_folder, topdown=False))[-1][-1]:
        ...
        im_dir = im_folder + '//' + im_name # generate data folder
        im_dir2 = im_folder2 + '//' + im_name # generate data folder

        noise_im = tiff.imread(im_dir) # read target image
        noise_im2 = tiff.imread(im_dir2)
        ...
        noise_im2 = (noise_im2-noise_im2.min()).astype(np.float32)/args.normalize_factor
        ...
        for x in range(0,int((whole_h-img_h+gap_h)/gap_h)):
            for y in range(0,int((whole_w-img_w+gap_w)/gap_w)):
                for z in range(0,int((whole_s-img_s2+gap_s2)/gap_s2)):
                    ...
                    noise_patch1 = noise_im[init_s:end_s:2,init_h:end_h,init_w:end_w] # raw
                    noise_patch2 = noise_im[init_s+1:end_s:2,init_h:end_h,init_w:end_w] # gt, note the gt is the next frame

                    patch_name = args.datasets_folder+'_x'+str(x)+'_y'+str(y)+'_z'+str(z)
                    train_raw.append(noise_patch1.transpose(1,2,0)) # append raw image
                    train_GT.append(noise_patch2.transpose(1,2,0))  # append raw image
                    name_list.append(patch_name) # name list, as 
...
def train_preprocess_lessMemory(args):
    ...
    im_folder2 = args.datasets_path+'//'+args.datasets_folder + '_gt'
    ...
    for im_name in list(os.walk(im_folder, topdown=False))[-1][-1]:
        ...
        if noise_im.shape[0]>args.select_img_num:
            ...
            noise_im2 = noise_im2[0:args.select_img_num,:,:]

        gap_s2 = get_gap_s(args, noise_im) # get gap
        # print('noise_im shape -----> ',noise_im.shape)
        # print('noise_im max -----> ',noise_im.max())
        # print('noise_im min -----> ',noise_im.min())
        noise_im = (noise_im-noise_im.min()).astype(np.float32)/args.normalize_factor
        ...
        for x in range(0,int((whole_h-img_h+gap_h)/gap_h)):
            for y in range(0,int((whole_w-img_w+gap_w)/gap_w)):
                for z in range(0,int((whole_s-img_s2+gap_s2)/gap_s2)):
                    ...
                    # print(' single_coordinate -----> ',single_coordinate)
                    coordinate_list[patch_name] = single_coordinate
    return  name_list, noise_im, noise_im2, coordinate_list

def train_preprocess_lessMemoryMulStacks_eval(args):
    img_h = args.img_h
    img_w = args.img_w
    img_s = args.img_s
    ...
    gap_w = args.gap_w
    gap_s2 = args.gap_s*2
    im_folder = args.datasets_path + '//' + args.datasets_folder_eval
    im_folder2 = args.datasets_path  + '//' + args.datasets_folder_eval + '_gt'
    ...
    stack_num = len(list(os.walk(im_folder, topdown=False))[-1][-1]) # find all training files under the target folder.
    print('Total number -----> ', stack_num)
    print('Total number -----> ', img_h,img_w,img_s,gap_h,gap_w,gap_s2)

    stack_num = len(list(os.walk(im_folder, topdown=False))[-1][-1])
    img_num_per_stack = math.ceil(args.train_datasets_size/stack_num)
    for im_name in list(os.walk(im_folder, topdown=False))[-1][-1]:
        print(im_name) # print file name
        im_dir = im_folder + '//' + im_name # generate data folder
        im_dir2 = im_folder2 + '//' + im_name # generate data folder
        ...
        for ii in range(0, img_num_per_stack):
            ...
            single_coordinate['end_s'] = end_s
            # noise_patch1 = noise_im[init_s:end_s,init_h:end_h,init_w:end_w]
            patch_name = args.datasets_folder_eval+'_'+im_name.replace('.tif','')+'_x'+str(init_h)+'_y'+str(init_w)+'_z'+str(init_s)
...
def test_preprocess_lessMemoryNoTail_chooseOne (args, N):
    ...
    im_folder = args.datasets_path+'//'+args.datasets_folder +'//mov_w_bg'
    ...
    if input_pretype == 'mean':
        print('input_pretype == mean')
        input_img_ave_single = np.mean(noise_im, axis=0)
        input_img_ave = np.zeros(noise_im.shape)
        for i in range(0, noise_im.shape[0]):
            input_img_ave[i,:,:] = input_img_ave_single
        noise_im = noise_im-input_img_ave
