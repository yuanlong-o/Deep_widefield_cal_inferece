...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='cr', num_groups=8, down_num=4,
                 **kwargs):
        super(UNet3D, self).__init__()

        if isinstance(f_maps, int):
            # use 4 levels in the encoder path as suggested in the paper
            f_maps = create_feature_maps(f_maps, number_of_fmaps=down_num)
...
class UNet3D_big(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='cr', num_groups=8, down_num=4,
                 **kwargs):
        super(UNet3D_big, self).__init__()
        ...
        # in the last layer a 1×1 convolution reduces the number of output
        # channels to the number of labels
        self.final_conv = nn.ConvTranspose3d(f_maps[0], out_channels, 2, 2)

        self.init_conv = nn.Conv3d(in_channels, in_channels, 5, 2, 2)
