...
##############################################################################################################################################
##############################################################################################################################################
def FFDrealign(input, rate):
    # rate = 3
    # batch channel time height width
    realign_input = torch.cuda.FloatTensor(input.shape[0], input.shape[1]*rate*rate, input.shape[2], int(input.shape[3]/rate), int(input.shape[4]/rate))
    # print('realign_input -----> ',realign_input.shape)
    # print('input -----> ',input.shape)
    for i in range(0, rate):
        for ii in range(0, rate):
            rr = rate*i+ii
            realign_input[:,rr,:,:,:] = input[:,:,:,i::rate, ii::rate]

    return realign_input


def inv_FFDrealign(input, rate):
    # batch channel time height width
    realign_input = torch.cuda.FloatTensor(input.shape[0], int(input.shape[1]/rate/rate), input.shape[2], int(input.shape[3]*rate), int(input.shape[4]*rate))

    for i in range(0, rate):
        for ii in range(0, rate):
            rr = rate*i+ii
            realign_input[:,:,:,i::rate, ii::rate] = input[:,rr,:,:,:]
...
def inv_FFDrealign8(input):
    # batch channel time height width
    realign_input = torch.cuda.FloatTensor(input.shape[0], int(input.shape[1]/4), int(input.shape[2]*2), int(input.shape[3]*2), int(input.shape[4]*2))
